...
# Coba untuk mengimpor modul yang diperlukan
try:
    # Load the saved model
    model = pickle.load(open('penyakit_jantung.sav', 'rb'))
except FileNotFoundError:
    st.error("File model tidak ditemukan. Silakan periksa jalur file.")
    st.stop()
except ModuleNotFoundError as e:
    st.error(f"Terjadi kesalahan: {str(e)}. Pastikan semua dependensi terpasang.")
    st.stop()
